...
public class CRC32Or16 {

    static final int[] table16 =
            {
    ...
    public static int getCrc(byte[] data) {
        ...
        for (int i = 0; i < length; i++) {
            crc = ((crc & 0xff) << 8) ^ table16[(((crc & 0xff00) >> 8) ^ data[i]) & 0xff];
        }
        crc = crc & 0xffff;
        return crc;
    }
   ...
   /* public static void main(String[] strings){
        int[] data = new int[]{0x11,0x7a,
                0x00,0x00, (byte) 0xa8,0x22,0x00,0x20,0x01,0x59,0x00,0x08,0x09,0x59,0x00,0x08,0x0b,0x59,0x00,0x08,0x0d,0x59,0x00,0x08,0x0f,0x59,0x00,0x08,0x11,0x59,0x00,0x08
                ,0x00,0x00,0x00 ,0x00 ,0x00 ,0x00 ,0x00 ,0x00 ,0x00 ,0x00 ,0x00 ,0x00 ,0x00 ,0x00 ,0x00 ,0x00 ,0x13 ,0x59,0x00 ,0x08 ,0x15 ,0x59 ,0x00 ,0x08 ,0x00 ,0x00 ,0x00 ,0x00
                ,0x17 ,0x59 ,0x00 ,0x08 ,0x19 ,0x59 ,0x00 ,0x08 ,0x1b ,0x59 ,0x00 ,0x08 ,0x1b ,0x59 ,0x00 ,0x08 ,0x1b ,0x59 ,0x00 ,0x08 ,0x1b ,0x59 ,0x00 ,0x08 ,0x1b ,0x59 ,0x00 ,0x08
                ,0x1b ,0x59 ,0x00 ,0x08 ,0x1b ,0x59 ,0x00 ,0x08 ,0x1b ,0x59 ,0x00 ,0x08 ,0x1b ,0x59 ,0x00 ,0x08 ,0x1b ,0x59 ,0x00 ,0x08 ,0x1b ,0x59 ,0x00 ,0x08 ,0x1b ,0x59 ,0x00 ,0x08
                ,0x1b ,0x59 ,0x00 ,0x08 ,0x1b ,0x59 ,0x00 ,0x08
        };
        System.out.println(Arrays.toString(HexString.TowByteArray(getCrc(data))));;
    }*/

    public static int CalcCRC16(int[] data) {
        int crc = 0;
        for (int i = 0; i < data.length; i++) {
            crc = (crc << 8) ^ table16[(crc >> 8) ^ data[i]];
        }
        return crc;
    }


    public static int CalcCRC16Ex(int crc, int data[]) {
        for (int i = 0; i < data.length; i++) {
            crc = (crc << 8) ^ table16[(crc >> 8) ^ data[i]];
        }
        return crc;
    ...
    public static int CalcCRC32(byte[] data) {
        int crc = 0;
        for (int i = 0; i < data.length; i++) {
            crc =  ((crc << 8) ^ table32[(crc >>> 24) ^ data[i] & 0xff]);
        }
        return crc;
    }


/*    public static int CalcCRC321(byte[] data) {
        int crc = 0xffffffff;
        for (int j = 0; j < data.length ; j++) {
            crc = (crc << 8) ^ table32[((crc >> 24) ^ data[j]) & 0xFF];
        }
        return crc;
    }*/
...
/*    public static int getCRC32(byte[] data){
        int crc = 0xffffffff;
        for (byte b : data) {
            crc = (crc >>>8 ^ table32[(crc ^ b) & 0xff]);
        }
        crc = crc ^ 0xffffffff;
        return crc;
    }*/

    public static int CalcCRC32Ex(int crc, int[] data) {
        for (int i = 0; i < data.length; i++) {
            crc = (crc << 8) ^ table32[(int) ((crc >> 24) ^ data[i])];
        }
        return crc;
    }
