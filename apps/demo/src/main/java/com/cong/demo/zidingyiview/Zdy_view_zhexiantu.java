...
public class Zdy_view_zhexiantu extends View {
    ...
    private List<ZhexiantuData> data = new ArrayList<ZhexiantuData>();
    ...
    public Zdy_view_zhexiantu(Context context) {
        this(context, null);
    }

    public Zdy_view_zhexiantu(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public Zdy_view_zhexiantu(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        if (changed) {
            ...
            firstMinX = mWidth - originX - (data.size() - 1) * intervalX - leftRightExtra;
...
//        drawLegend(canvas);
    ...
    private void drawX(Canvas canvas) {
        ...
        for (int i = 0; i < data.size(); i++) {
            ...
            // x轴上的文字
            canvas.drawText(
                    data.get(i).getXValues(), 
                    firstPointX + i * intervalX - getTextWidth(paintText, "17.01") / 2,
                    originY + textToXYAxisGap + getTextHeight(paintText, "17.01"), 
                    paintText);
        }
    ...
    private void drawBrokenLine(Canvas canvas) {
        canvas.save();
        // y轴文字
        minValueY = data.get(0).getYValues();
        for (int i = 0; i < data.size(); i++) {
            // 找出y轴的最大最小值
            if (data.get(i).getYValues() > maxValueY) {
                maxValueY = data.get(i).getYValues();
            }
            if (data.get(i).getYValues() < minValueY) {
                minValueY = data.get(i).getYValues();
            }
        }
        // 画折线
        ...
        for (int i = 0; i < data.size(); i++) {
            if (i == 0) {
                path.moveTo(firstPointX, mHeight - paddingDown - leftRightExtra - data.get(i).getYValues() * aver + minValueY * aver);
            } else {
                path.lineTo(firstPointX + i * intervalX, mHeight - paddingDown - leftRightExtra - data.get(i).getYValues() * aver + minValueY * aver);
            }
        }
        ...
        for (int i = 0; i < data.size(); i++) {
            canvas.drawCircle(firstPointX + i * intervalX,
                    mHeight - paddingDown - leftRightExtra - data.get(i).getYValues() * aver + minValueY * aver, bigCircleR, paintBlue);
            canvas.drawCircle(firstPointX + i * intervalX,
                    mHeight - paddingDown - leftRightExtra - data.get(i).getYValues() * aver + minValueY * aver, smallCircleR, paintBack);
        }
        //将折线超出x轴坐标的部分截取掉（左边）
...
//        canvas.drawRect(rectF, paintBack);
    ...
    public void setData(List<ZhexiantuData> data) {
        this.data.clear();
